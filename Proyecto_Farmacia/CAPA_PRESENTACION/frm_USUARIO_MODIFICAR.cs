...
{
    public partial class frm_USUARIO_MODIFICAR : Form
    {
        public frm_USUARIO_MODIFICAR()
        {
            InitializeComponent();
        }
        private string resp = "";
        private void frm_USUARIO_MODIFICAR_Load(object sender, EventArgs e)
        {
            txtNOMBRE.Text = Program.nombre;
            txtAPELLIDO.Text = Program.apellido;
            txtTELEFONO.Text = Program.telefono;
            comboBox1.Text = Program.sexo;
            txtOBSERVACION.Text = Program.observacion;
            txtUSUARIO.Text = Program.usuario;
            txtCONTRASEÑA.Text = Program.pass;
